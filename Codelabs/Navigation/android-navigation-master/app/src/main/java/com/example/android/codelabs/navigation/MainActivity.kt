...
//        sideNavView?.setupWithNavController(navController)
        // TODO END STEP 9.4
        val sideNavigator = findViewById<NavigationView>(R.id.nav_view)
        sideNavigator?.setupWithNavController(navController)
    }
...
//                || super.onOptionsItemSelected(item)
        // TODO END STEP 9.2
        // we don't have a view that is a child of the nav controller here, so to find it we must
        // pass in the nav host fragment resource itself.
        return item.onNavDestinationSelected(findNavController(R.id.my_nav_host_fragment))
                || super.onOptionsItemSelected(item)
    }
