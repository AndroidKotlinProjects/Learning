...
class RefreshMainDataWork(context: Context, params: WorkerParameters, private val network: MainNetwork) :
    ...
    override suspend fun doWork(): Result {
        /* Notice how we use a getDatabase() function to fetch the database singleton but we just
        * make a new TitleRepository() as we need it. This is because the database is a much
        * heavier resource. The Repository on the other hand has no reason to be a singleton. */
        val database = getDatabase(applicationContext)
        val repository = TitleRepository(network, database.titleDao)

        /* Try refresh the title from the network into the repository */
        return try {
